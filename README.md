
# Hi, I'm Rupali 

> Cyber Security student at VIT Bhopal, learning to break things responsibly and build tools that matter — mostly in **Python**.



## Stack

| Skill | Level |
|---|---|
| 🐍  Python | `main` |
| 🛡️ Cyber Security | `learning` |
| ➕ C++ | `learning` |
| 🤖 Machine Learning | `learning` |
| 🌐 Web Dev | `exploring` |

---

## About

- 🎓 **VIT Bhopal**
- 📍 **India**
- 📅 **1st Year, CSE — Cyber Security and digital forensics**
- 🔭 **Building in public**
- 📫 **Open to collaborate**

---

## Currently working towards

```
01  Write Python scripts for real security tasks — scanners, sniffers, automations
02  Understand the offensive side: CTF challenges and ethical hacking fundamentals
03  Apply ML to security problems like intrusion detection and anomaly detection
04  Ship projects that I'm genuinely proud to show
```

---

## 💻 Tech Stack

![Python](https://img.shields.io/badge/Python-3670A0?style=for-the-badge&logo=python&logoColor=ffdd54)
![C++](https://img.shields.io/badge/C++-00599C?style=for-the-badge&logo=c%2B%2B&logoColor=white)

---

## 📊 GitHub Stats

📊 GitHub Stats
<p align="center">
  <img src="https://github-readme-stats-fast.vercel.app/api?username=RUPALI498&show_icons=true&hide_border=true&count_private=true&theme=default" height="150"/>
  &nbsp;
  <img src="https://github-readme-stats-fast.vercel.app/api/top-langs/?username=RUPALI498&layout=compact&hide_border=true&theme=default" height="150"/>
</p>
<p align="center">
  <img src="https://streak-stats.demolab.com?user=RUPALI498&hide_border=true&theme=default"/>
</p>


---

<p align="right"><i>"security is a process, not a product"</i></p>
